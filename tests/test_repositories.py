...
@pytest.mark.git
def test_fetch_user_repo_data(github_session, request):
    base_url = request.config.getoption("--url")
    response = fetch_repo(github_session, base_url)
    assert response.status_code==200

@pytest.mark.git2
def test_create_repo(github_session,request):
    base_url = request.config.getoption("--url")
...
@pytest.mark.git2
def test_update_repo(github_session,request):
    base_url = request.config.getoption("--url")
    response = update_repo(github_session,base_url)
    assert response.status_code == 200

@pytest.mark.git2
def test_delete_repo(github_session,request):
    base_url = request.config.getoption("--url")
